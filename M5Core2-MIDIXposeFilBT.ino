...
enum DisplayMode {
  ...
  BASE_SET_MODE        // A long-press (Phase 2): pick transpose base reference
};
...
// ============================================================
// Schema (all keys optional — missing keys fall back to built-in defaults):
...
//   MidiInputSource: USB|MIDIIN|MIX  (Tab5 only — accepted but ignored on Core2)
//   MajorUpperTranspose: bool        (KEY-mode behavior)
//   BTAutoReconnect: bool
//   ShowSplash: bool
//
// SD未挿入 / config.jsonなし / パース失敗 はすべて「default動作と同じ」として
// 静かに継続する。ブートを止めない。
struct DeviceConfig {
  char defaultApp[16];
...
void setup() {
  ...
  // /config.json を SD から読む。SD未挿入や config.json が無い場合は
  // built-in default で続行 (loadDeviceConfigFromSD は false 返すだけ)。
  setDefaultConfig();
  ...
  // Apply config now (sets initial transpose, flags, range, default mode).
  applyDeviceConfig();
  drawInterface();
